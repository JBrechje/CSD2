...
- Run the code, read the code and answer the following question:
  ...
  adhv de time signatur (in dit geval 3/4) wordt er berkent hoeveel noten daar in passen (dus 12)
  vervolgens wordt er in de sequence gezecht op welke stappen een noot komt (dus 0, 2, 4, 8, 11) 

- Alter the code:
  Fix the bug that occurs: "IndexError: pop from empty list"
  pop(0) kijkt naar het eerste element van de lijst en verwijdert die dus als er geen elementen meer in de lijst zitten kan dit niet
  pop() verwijdert het laatste element van de lijst dus dit kan wel als er nog maar 1 element in de lijst zit
"""
